# BAR-119 — WeaveForge pilot: Verify phase exit gate

## Goal
Apply BAR to WeaveForge with orchestration/adversarial profile. This atomic step focuses on: verify phase exit gate.
...
Demonstrate end-to-end that the WeaveForge pilot phase accomplishes: Apply BAR to WeaveForge with orchestration/adversarial profile. Run a deliberate defect/failure proving the phase catches incorrect behavior, then restore/fix it and prove recovery. Freeze the phase evidence before proceeding.

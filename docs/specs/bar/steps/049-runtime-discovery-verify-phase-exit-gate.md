# BAR-049 — Runtime discovery: Verify phase exit gate

## Goal
Launch real stack and map observable services, routes, APIs, streams and topology. This atomic step focuses on: verify phase exit gate.
...
Demonstrate end-to-end that the Runtime discovery phase accomplishes: Launch real stack and map observable services, routes, APIs, streams and topology. Run a deliberate defect/failure proving the phase catches incorrect behavior, then restore/fix it and prove recovery. Freeze the phase evidence before proceeding.

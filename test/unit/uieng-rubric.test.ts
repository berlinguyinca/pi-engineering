...
describe("uieng 60-metric versioned rubric", () => {
  ...
  it("every metric is a fully-typed entry with all required fields", () => {
    for (const m of METRICS) {
      ...
      assert.ok(["info", "low", "medium", "high", "critical"].includes(m.severity), `${m.id} severity invalid`);
  ...
  it("scoreRecord clamps score/confidence and honors overrides", () => {
    ...
    assert.throws(() => scoreRecord("contrast", { score: Number.NaN }), /Invalid score/);
  });
  ...
  it("scoreAll rejects missing and unknown metric ids", () => {
    ...
    assert.throws(() => scoreAll({ ...inputs, extra: { score: 10 } }), /Unknown metric id "extra"/);
  });

  it("aggregate is explicitly secondary and retains every individual score", () => {
    // Scores 1..60 (no clamping) to keep aggregate math deterministic.
    const scores: MetricScore[] = METRIC_IDS.map((id, i) => scoreRecord(id, { score: i + 1 }));
    ...
    assert.equal(agg.mean_score, 30.5);
    assert.equal(agg.min_score, 1);
    assert.equal(agg.max_score, 60);
    assert.equal(agg.median_score, 30.5);

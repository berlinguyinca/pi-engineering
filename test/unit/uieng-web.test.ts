...
import type { Candidate, EvaluationRun, ExecutionProvenance, TaskRequest } from "../../src/uieng/schemas.ts";
...
describe("buildWebSnapshot", () => {
  ...
  it("computes per-metric deltas deterministically and sorted by metric id", () => {
    const deltas = computeMetricDeltas(
      { contrast: 50, visual_hierarchy: 60 },
      { contrast: 90, visual_hierarchy: 75 },
  ...
  it("all event envelopes carry schema_version, type, id, at, provenance", () => {
    ...
    const events: UiengEvent[] = [
      uiengEvaluationCompleted({
        ...
        at,
        provenance: prov,
        run: baseInput().approvalDecisions[0]!.evaluation,
        dashboard: baseInput().dashboard,
      }),
...
describe("PiWebAdapter contract", () => {
  it("documents the stable query contract without an implementation", () => {
    const query: PiWebSnapshotQuery = {
      missionId: "M1",
      since: "2026-01-01T00:00:00Z",
      include: ["dashboard", "workQueue"],
    };
    ...
    const adapter = {
      name: "pi-web",
      getSnapshot: async (q: PiWebSnapshotQuery): Promise<WebSnapshot> =>
        buildWebSnapshot(baseInput({ snapshotId: q.snapshotId ?? "SNAP-Q" })),
      getWorkQueue: async () => [],

 ...
 *
 * - B, the trigger: on `agent_settled` (the run fully settled — never mid-run),
 *   if the context is past `window − tunedReserve`, call `ctx.compact()`.
 *   Debounced: after triggering, it re-arms only once the context has dropped
 *   back below the threshold, so a compaction that fails or does not shrink the
 *   context cannot loop.
...
export function loadPiPrepareCompaction(): Promise<PrepareCompaction | undefined> {
  prepareLoad ??= (async () => {
    try {
      ...
      if (typeof resolveSpecifier !== "function") return undefined;
      const entry = resolveSpecifier("@earendil-works/pi-coding-agent");
...
/**
 * Register B (trigger) and A (content) on a Pi extension host. Returns false
 * when the running Pi is too old — then nothing is registered, and a one-time
 * notice says why.
 */
export function registerAutoCompaction(pi: AutoCompactionHost, options: AutoCompactionOptions = {}): boolean {
  ...
  const resolve = (ctx: AutoCompactionContext): EffectiveCompaction | undefined => {
    ...
    const effective = effectiveCompaction(
      model,
  ...
  const onModelChange = (_event: unknown, ctx: AutoCompactionContext) => {
    // A new model means a new threshold: start armed and announce its values.
    state = { armed: true };
    resolve(ctx);
  };
  ...
  // B: trigger at the tuned threshold, only once the run has fully settled.
  pi.on("agent_settled", ((_event: unknown, ctx: AutoCompactionContext) => {
    const effective = resolve(ctx);
    if (!effective?.enabled || effective.reserveSource === "default") return;
    ...
    if (!decision.compact || !ctx.isIdle()) return;
    ctx.compact({
      onError: (error) =>
        emitTelemetry({
          level: "warning",
          key: "compaction-autotune:failed",
          text: `Tuned compaction did not complete: ${error.message}`,
        }),
    });
  }) as never);

  // A: every compaction honours the tuned keepRecent and summary budget.
  pi.on("session_before_compact", (async (event: BeforeCompactEvent, ctx: AutoCompactionContext) => {
    const effective = resolve(ctx);
    const model = ctx.model;
    if (!effective || !model) return undefined;
    ...
    try {
      ...
      if (prepare) {
        // Nothing to summarize at the tuned keepRecent: let Pi's own cut run.
        preparation = prepare(event.branchEntries, settings);
        ...
        preparation = {
          ...event.preparation,
          settings: { ...event.preparation.settings, reserveTokens: settings.reserveTokens },
        };
      }
      // Through the session's registry: the composed provider (gateway wrapper,
      // thinking-off policy) and Pi's own auth resolution.
      const streamFn = ((m: never, c: never, o?: never) => ctx.modelRegistry.streamSimple(m, c, o)) as never;
      const compaction: CompactionResult = await compact(
